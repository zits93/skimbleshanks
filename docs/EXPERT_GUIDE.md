...
2. **환경 변수 설정**:
   API 호출 시 필요한 인증 키와 CORS 허용 도메인을 설정합니다. 본인이 설정한 `SKIMBLE_API_KEY`를 웹 GUI의 설정 창에도 동일하게 입력해야 합니다.

   | 환경 변수 | 설명 | 기본값 |
   | :--- | :--- | :--- |
   | `SKIMBLE_API_KEY` | API 호출 시 필요한 인증 키 (헤더 `X-API-KEY`) | (필수) |
...
ngrok http 8000
```
생성된 ngrok 주소를 웹 GUI의 **API 설정** 창에 입력하여 사용하세요.
